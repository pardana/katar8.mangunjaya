...
class M_bill extends CI_Model {
    ...
    public function get_bill($cid){
        $this->db->select("rq.req_number, dl.bl_number, dl.id AS dlid, mc.company_name AS co_name, dl.ff_name, dl.amount, ms.status_desc, rq.created_date");
        ...
        $this->db->where("dl.slid", $cid);
        $this->db->where("rq.status_id != 1");
        $this->db->where("rq.ff_reject != 1");
	...
	public function get_view($id){
        $this->db->select("rq.id AS rqid, rq.tag, rq.status_id, rq.notes_request, rq.notes_reject, dl.amount, dl.file_proforma, dl.bl_number, dl.id AS dlid, rq.req_number, mc.company_name AS co_name, dl.ff_name, ms.status_desc, mb.bank_name, dl.expired_do, dl.file_invoice, dl.file_do, rq.created_date");
        $this->db->from("request rq");
        $this->db->join("delivery dl", "dl.request_id = rq.id", "LEFT");
        $this->db->join("m_status ms", "ms.id_status = rq.status_id", "LEFT");
        $this->db->join("m_company mc", "mc.id_company = rq.company_id", "LEFT");
